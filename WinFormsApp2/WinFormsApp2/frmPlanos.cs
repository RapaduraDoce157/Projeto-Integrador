...
namespace WinFormsApp2
{
    ...
    public partial class frmPlanos : Form
    {
        ...
        {
            float vVenda;
            if (!float.TryParse(txtPreco.Text, out vVenda) && txtPreco.Text.Trim() != "")
            {
                MessageBox.Show("Erro. Valor de custo deve ser numérico e sem formato.", "Ifood de PC");
                txtPreco.Text = "";
                txtPreco.Focus();
                return;
            }
            ...
            txtPreco.Text = String.Format("{0:C}", vVenda);
        }
